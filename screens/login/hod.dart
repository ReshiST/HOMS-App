...
class HODLoginPage extends StatefulWidget {
  @override
  _HODLoginPageState createState() => _HODLoginPageState();
}
...
class _HODLoginPageState extends State<HODLoginPage> {
  ...
  Future<void> login() async {
    ...
    final response = await http.post(
      Uri.parse("https://reshist.me/homspwa/api/hod-login.php"), 
      body: {
        "username": email,
        "password": password,
      },
    ...
    if (response.statusCode == 200) {
      ...
      if (data["status"] == "success") {
        ...
        Navigator.pushNamed(context, '/hod_dashboard'); 
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('HoD Advisor Login', style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.bold)),
        centerTitle: true,
        elevation: 0,
