...
import '../../main.dart';
class HODApprovedPassScreen extends StatefulWidget {
  @override
  _HODApprovedPassScreenState createState() => _HODApprovedPassScreenState();
}

class _HODApprovedPassScreenState extends State<HODApprovedPassScreen> {
  ...
  Future<void> fetchPassData() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://reshist.me/homspwa/api/hod-approved-pass.php'),
        body: {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          setState(() {
            isLoading = false;
            errorMessage = 'API Response Error: ${data['message'] ?? 'Unknown Error'}';
          });
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(errorMessage)),
          );
