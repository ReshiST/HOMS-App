...
import '../../main.dart';
class VPApprovedPass extends StatefulWidget {
  @override
  _VPApprovedPassState createState() => _VPApprovedPassState();
}

class _VPApprovedPassState extends State<VPApprovedPass> {
  List<Map<String, String>> passHistory = [];

  // Fetch pass data from API
  Future<List<Map<String, String>>> fetchPassData() async {
    final response = await http.get(Uri.parse('https://reshist.me/homspwa/api/vp-approved-pass.php'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Approved Pass', style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.bold)),
                ...
                subtitle: Column(
                  ...
                  children: [
                    ...
                    Text('To: ${pass['toDate']} ${pass['inTime']}', style: GoogleFonts.poppins(fontSize: 14)),
                     Text('Principal/VP Status: ${currentStatusDetails['text']}',
                    ...
                    // Current Status
                    Text('Current Status: ${vpStatusDetails['text']}',
